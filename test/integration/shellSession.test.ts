...
describe("PersistentShellSession", () => {
  ...
  it("能在持久化 POSIX shell 中跨命令保留 cwd", async () => {
    ...
    expect(await realpath(normalizeShellPath(first.stdout))).toBe(realRoot);
    expect(await realpath(normalizeShellPath(second.stdout))).toBe(path.join(realRoot, "child"));
  });
  ...
  it("能通过 POSIX shell 读取项目和全局 skill 资源", async () => {
    ...
    expect(projectSkill.stdout).toContain("项目正文标记：pdf-processing");
    expect(projectReference.stdout).toContain("PDF 参考");
    expect(globalScript.stdout).toContain("模拟 API 请求");
  });
  ...
  it("能在原生 shell 中执行命令并正确回报 cwd", async () => {
    ...
    expect(normalizeShellPath(result.stdout)).toBe(root);
    expect(normalizeShellPath(result.cwd)).toBe(root);
  });
